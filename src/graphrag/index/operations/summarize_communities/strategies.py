...
async def _run_extractor(
    ...
    try:
        ...
        logger.info(f"COMMUNITY DEBUG: Acquiring rate limiter (100k per 60s - no limit)")
        await rate_limiter.acquire()
        rate_limit_end = time.time()
        logger.info(f"COMMUNITY DEBUG: Rate limiter acquired in {rate_limit_end - rate_limit_start:.2f} seconds")

        extractor_start = time.time()
        logger.info(f"COMMUNITY DEBUG: Starting community extractor")
        results = await extractor(input)
        extractor_end = time.time()
        logger.info(f"COMMUNITY DEBUG: Community extractor completed in {extractor_end - extractor_start:.2f} seconds")
