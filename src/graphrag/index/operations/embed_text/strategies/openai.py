...
async def run(
    ...
    # CRITICAL DEBUG: Log the token limits being used
    logger.info(f"GRAPHRAG EMBEDDING STRATEGY: batch_max_tokens = {batch_max_tokens}")
    logger.info(f"GRAPHRAG EMBEDDING STRATEGY: batch_size = {batch_size}")

    llm_config = args["llm"]
    llm_config = LanguageModelConfig(**args["llm"])
    splitter = _get_splitter(llm_config, batch_max_tokens)

    # CRITICAL DEBUG: Log splitter configuration
    logger.info(f"GRAPHRAG EMBEDDING STRATEGY: TokenTextSplitter chunk_size = {batch_max_tokens}")
    logger.info(f"GRAPHRAG EMBEDDING STRATEGY: TokenTextSplitter encoding = {llm_config.encoding_model}")
