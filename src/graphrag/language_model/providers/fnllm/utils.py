...
def _create_openai_config(config: LanguageModelConfig, azure: bool) -> OpenAIConfig:
    ...
    # CRITICAL DEBUG: Log the exact config being processed
    logger.debug(f"FNLLM DEBUG: _create_openai_config called with azure={azure}")
    logger.debug(f"FNLLM DEBUG: config.api_base = '{config.api_base}'")
    logger.debug(f"FNLLM DEBUG: config.model = '{config.model}'")
    logger.debug(f"FNLLM DEBUG: config.type = '{config.type}'")
    ...
    # CRITICAL DEBUG: Log exactly what's being passed to PublicOpenAIConfig
    logger.debug(f"FNLLM DEBUG: Creating PublicOpenAIConfig with base_url='{config.api_base}'")
    logger.debug(f"FNLLM DEBUG: Creating PublicOpenAIConfig with model='{config.model}'")
    logger.debug(f"FNLLM DEBUG: Creating PublicOpenAIConfig with api_key={'***' if config.api_key else 'None'}")

    openai_config = PublicOpenAIConfig(
        api_key=config.api_key,
    ...
    # CRITICAL DEBUG: Log the created config
    logger.debug(f"FNLLM DEBUG: Created PublicOpenAIConfig with base_url='{openai_config.base_url}'")
...
def run_coroutine_sync(coroutine: Coroutine[Any, Any, T]) -> T:
    ...
    logger.debug(f"SYNC DEBUG: Starting run_coroutine_sync")

    if not _thr.is_alive():
        thread_start = time.time()
        logger.debug(f"SYNC DEBUG: Starting async thread")
        _thr.start()
        thread_started = time.time()
        logger.debug(f"SYNC DEBUG: Async thread started in {thread_started - thread_start:.2f} seconds")

    schedule_start = time.time()
    logger.debug(f"SYNC DEBUG: Scheduling coroutine on async thread")
    future = asyncio.run_coroutine_threadsafe(coroutine, _loop)
    schedule_end = time.time()
    logger.debug(f"SYNC DEBUG: Coroutine scheduled in {schedule_end - schedule_start:.2f} seconds")

    result_start = time.time()
    logger.debug(f"SYNC DEBUG: Waiting for coroutine result")
    result = future.result()
    result_end = time.time()
    logger.debug(f"SYNC DEBUG: Coroutine result received in {result_end - result_start:.2f} seconds")

    sync_end = time.time()
    logger.debug(f"SYNC DEBUG: Total run_coroutine_sync completed in {sync_end - sync_start:.2f} seconds")
    return result
