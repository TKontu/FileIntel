...
class OpenAIChatFNLLM:
    ...
    async def achat(
        ...
        logger.info(f"FNLLM DEBUG: Starting achat call, prompt length: {len(prompt)}")

        if history is None:
            model_start = time.time()
            logger.info(f"FNLLM DEBUG: Calling self.model() without history")
            response = await self.model(prompt, **kwargs)
            model_end = time.time()
            logger.info(f"FNLLM DEBUG: self.model() call completed in {model_end - model_start:.2f} seconds")
        else:
            model_start = time.time()
            logger.info(f"FNLLM DEBUG: Calling self.model() with history (length: {len(history)})")
            response = await self.model(prompt, history=history, **kwargs)
            model_end = time.time()
            logger.info(f"FNLLM DEBUG: self.model() call completed in {model_end - model_start:.2f} seconds")

        response_creation_start = time.time()
        logger.info(f"FNLLM DEBUG: Creating BaseModelResponse")
        ...
        logger.info(f"FNLLM DEBUG: BaseModelResponse creation completed in {response_creation_end - response_creation_start:.2f} seconds")

        fnllm_end = time.time()
        logger.info(f"FNLLM DEBUG: Total achat completed in {fnllm_end - fnllm_start:.2f} seconds")
        return result
...
class AzureOpenAIChatFNLLM:
    ...
    async def achat(
        ...
        logger.info(f"FNLLM AZURE DEBUG: Starting achat call, prompt length: {len(prompt)}")

        if history is None:
            model_start = time.time()
            logger.info(f"FNLLM AZURE DEBUG: Calling self.model() without history")
            response = await self.model(prompt, **kwargs)
            model_end = time.time()
            logger.info(f"FNLLM AZURE DEBUG: self.model() call completed in {model_end - model_start:.2f} seconds")
        else:
            model_start = time.time()
            logger.info(f"FNLLM AZURE DEBUG: Calling self.model() with history (length: {len(history)})")
            response = await self.model(prompt, history=history, **kwargs)
            model_end = time.time()
            logger.info(f"FNLLM AZURE DEBUG: self.model() call completed in {model_end - model_start:.2f} seconds")

        response_creation_start = time.time()
        logger.info(f"FNLLM AZURE DEBUG: Creating BaseModelResponse")
        result = BaseModelResponse(
            output=BaseModelOutput(
                content=response.output.content,
                full_response=response.output.raw_model.to_dict(),
        ...
        response_creation_end = time.time()
        logger.info(f"FNLLM AZURE DEBUG: BaseModelResponse creation completed in {response_creation_end - response_creation_start:.2f} seconds")

        fnllm_azure_end = time.time()
        logger.info(f"FNLLM AZURE DEBUG: Total achat completed in {fnllm_azure_end - fnllm_azure_start:.2f} seconds")
        return result
