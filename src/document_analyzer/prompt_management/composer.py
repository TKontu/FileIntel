...
class PromptComposer:
    ...
    def compose(self, task_name: str, context: dict) -> str:
        ...
        if 'prompt' not in templates:
            raise ConfigException(f"Main 'prompt.md' template not found in task '{task_name}'")
        ...
        prompt_shell = self.template_engine.render_string(prompt_frame, **overhead_context)
        ...
        if 'document_text' in context:
            ...
            if len(document_tokens) > available_tokens:
                ...
                render_context['document_text'] = truncated_text + "\n... [TRUNCATED DUE TO CONTEXT LIMIT] ..."
            
        # Render the final prompt with the (potentially truncated) context
        final_prompt = self.template_engine.render_string(prompt_frame, **render_context)
