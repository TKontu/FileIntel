...
class UnifiedDocumentProcessor:
    ...
    def _validate_result(self, result: ProcessingResult) -> bool:
        ...
        total_text = " ".join([e.content for e in text_elements if isinstance(e.content, str)])
...
class DocumentToLLMBridge:
    ...
    async def prepare_for_llm(self, file_path: str, preserve_structure: bool = True) -> Dict[str, Any]:
    ...
    def _flattened_format(self, result: ProcessingResult) -> Dict[str, Any]:
        ...
        text_elements = [e for e in result.elements if e.type == "text"]
        full_text = "\n\n".join([e.content for e in text_elements if isinstance(e.content, str)])
...
async def main():
    ...
    simple_result = await bridge.prepare_for_llm(
        "simple_document.pdf", 
