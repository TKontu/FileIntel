...
anthropic_rate_limiter = RateLimiter(
    max_requests=settings.get("llm.anthropic.rate_limit", 15),
    per_seconds=60
)
...
class AnthropicProvider(LLMProvider):
    ...
    def generate_response(
        ...
        timeout: int = None,
        **kwargs,
    ) -> Generator[LLMResponse, None, None] if stream else LLMResponse:
        """
    ...
    def _generate_standard_response(self, prompt: str, model: str, timeout: int, **kwargs) -> LLMResponse:
        ...
        llm_response = LLMResponse(
            ...
            cost=cost,
            metadata={"usage": response.usage.dict(), "stop_reason": response.stop_reason}
        )
